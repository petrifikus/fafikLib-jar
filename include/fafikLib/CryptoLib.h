/** \brief Provides a class that lets you encrypt and decrypt raw data & small files
 *
 */

#ifndef FAFIK77_CRYPTOLIB_H
#define FAFIK77_CRYPTOLIB_H
...
class CryptoLib {
 public:
	static const int32_t _keyLocation;
	 ///TODO remove init()
	 CryptoLib(){ _init(); }
	~CryptoLib(){}
 ...
 #ifdef FAFIK77_UNIFILESTREAM_H	//re-implement those functions using "UniFileStream_base" streams for 64-bit && file support
		///warning output file will be slightly larger(up to 512 Bytes), but it might require a new cluster on disk
	bool encryptData_7(const UniFileStream_base& StreamIn);
...
//	inline void _DataOp_ShiftBitsData(bool side);
 ...
 #ifdef FAFIK77_UNIFILESTREAM_H	//re-implement those functions using "UniFileStream_base" streams
 #endif // FAFIK77_UNIFILESTREAM_H
