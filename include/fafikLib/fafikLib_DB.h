...
//	BYTE _b_TablesInherited= false;
 public:
  //def static val
	...
	static wxArrayString translate_StrToArr(const wxString& findKey_levelRecursive_str, const wxUniChar separator='.');

  //def later sub-classes
	struct itemEntry;
...
//		mode_write,
	...
	struct itemEntry
	{
	 ...
	 protected:
		...
		itemEntry_storeDerived _stores_derived= sdie_normal;
	 public:
		inline bool getAs_derived(itemEntry_Bytes** asDerived)	const {*asDerived= nullptr; if(sdie_bytes== _stores_derived)	return( *asDerived= (itemEntry_Bytes*)this );}
...
//			on ~delete:: {delete **linksTo, *linksTo= 0, //linksTo= 0 }
	// ]end new
	  //}end storage
		...
		iterator getOrAddIt( const wxString& key_, BYTE type_, bool caseInsensitive=true, bool *out_found=nullptr );
			///legacy call
		itemEntry* getOrAdd( const wxString& key_, BYTE type_, bool *out_found= nullptr, bool caseInsensitive=true ) { return *( getOrAddIt(key_, type_, caseInsensitive, out_found) ); }
...
//		}
		...
		itemEntry* findP( const wxString &item_byKey, itemStores type=ie_value, bool caseInsensitive= true ){
		...
		itemEntry* Add_back(const wxString& key_, const wxString& value_=wxEmptyString, BYTE type_stores_= 0);
...
//		inline itemEntry &get_ref() { return *get(); }
		...
		bool removeItem(const wxString& by_string, bool silent=false);
			///@return true if found removed
			///@param silent 0 -remove from table, 1 -nullify
		inline bool removeItem(size_t by_pos, bool silent=false);
			///@return true if found removed
			///@param silent 0 -remove from table, 1 -nullify
...
//		}
		...
		inline bool isCaseInsensitive() const{ if(type_tags & ie_caseInsensitive) return true; return false; }

		inline bool isWithBytes() const{ return(sdie_bytes== _stores_derived); }
//		inline bool isWithBytes() const {return _stores_derived== sdie_bytes; }
...
//		inline itemEntry* push_back(itemEntry *push){ return push_back_ptr(push);}
		...
		iterator AddIt( const wxString& key_, const wxString& value_=wxEmptyString, BYTE type_stores_= 0 );
		 ///adds *item into table or array depending on the type
		iterator AddItSmart( itemEntry* item );
			///adds new subItem with ( ... )
		itemEntry* Add( const wxString& key_, const wxString& value_=wxEmptyString, BYTE type_stores_= 0 );
			///@return true if ok, false if not "ie_auto_key"
			///adds new subItem with ( ... ), only when ie_auto_key
		itemEntry* Add( BYTE type_stores_, const wxString& value_ );
...
//		virtual wxLongLong Adv_getNumber_LL(){return 0;}
	...
	class itemEntry_Bytes: public itemEntry
	{
	 public:
		itemEntry_Bytes() {_stores_derived= sdie_bytes; }
		itemEntry_Bytes(fafikLib_readLineByte_base* IO, const wxFileOffset& start_p, const wxFileOffset& end_p)
			{_stores_derived= sdie_bytes; streamIO=IO; pos_curr=0; pos_start= start_p; pos_end= end_p; }
		 ///empty dtor as we dont allocate new
		virtual ~itemEntry_Bytes() {};

	 //easy functions
		void Open(fafikLib_readLineByte_base* IO)
			{streamIO=IO;}
		void Open(fafikLib_readLineByte_base* IO, const wxFileOffset& start_p, const wxFileOffset& end_p)
			{streamIO=IO; pos_curr=0; pos_start= start_p; pos_end= end_p; }
		void OpenSized(fafikLib_readLineByte_base* IO, const wxFileOffset& start_p, const wxFileOffset& size)
	 ...
	 //data store
		...
		fafikLib_readLineByte_base* streamIO= nullptr;
...
// TODO (main#9#06/01/18): Open( wxString file ) what if MDFC_Parted file???
	...
	BYTE Open(fafikLib_readLineByte_base* BaseIO, const wxString& UseInterpr, bool clearsPrevious= true);
...
//	#define Use_DataBase Open()
	...
	int loadInterpreterFile(fafikLib_readLineByte_base* item, const wxString& overwriteInterpreter= "fafikLib_PRON");
		///@return -1 on error, +int interpreters added
...
//]]
  ...
  //functions that will be used for jar like format .zip
	BYTE DataIntoFile();
//	///DataIntoFile()
...
//	#define import_FileIntoData FileIntoData
	...
	inline fafikLib_readLineByte_EOL getEolObj()const {return DB_fileIOLine->pass_useOnlyThisEndOL;}
	inline void setEolToType(fafikLib_readLineByte_EOL::fileLineEnd toType) {DB_fileIOLine->setEolToType(toType);}
	inline void setEolTo(const fafikLib_readLineByte_EOL& toEol) {DB_fileIOLine->setEolTo(toEol);}
 ...
 //data store
	settings thoseDB_settings;
 	...
 	fafikLib_readLineByte_file* DB_fileIO= nullptr;
 	wxFFile DB_file_write;
 	 ///more like an overriten virtual class
 	fafikLib_readLineByte_base* DB_fileIOLine= nullptr;
...
//	 ///redo into: use `*itemEntry_Stores` override instead of new element
...
//	interpreterStru &curr_interpreter= *curr_interpreter_p;
 ...
 public:
	...
	void UseEol(const fafikLib_readLineByte_EOL& useEol_);
