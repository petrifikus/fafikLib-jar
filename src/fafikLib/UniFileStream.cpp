...
//const wxFileOffset UniFileStream_base::invalidFileOffset= -1LL;

void UniFileStream_DetermineEOL( UniFileStream_base &baseClass, UniFileStream_EOL &EOL, const UniFileStream_EOL& onNotFound )
{
...
bool UniFileStream_base::readLineByte_base( wxString *line )
{
	reOpen();
	if( pass_useOnlyThisEndOL.isDefault() ){
		UniFileStream_DetermineEOL(*this, pass_useOnlyThisEndOL);
...
//printf("read_buff_str[1]= %u\n", read_buff_str[1]);
		wxString temp_wxStr( &temp_str.at(0) , wxConvUTF8, temp_str.size() );
		if(temp_str.size() && temp_wxStr.empty()){	//falls through (we want to force line content present)? 2024-02-10
			temp_wxStr.assign(temp_str.data(), temp_str.size());
		}
...
//wxLogMessage( "|%s|", *line );
		...
		wxString temp_wxStr( &complete_str->at(0) , wxConvUTF8, complete_str->size() );
		line->swap(temp_wxStr);
		read_buff_str.clear();
	} else {
		return false;
...
bool UniFileStream_file::readLineByte( wxString *p_line )
{
	reOpen();
	if(!p_line) p_line= pass_p_line;
	if(!p_line) return false;
	if(!pass_p_ffile) return false;
//	return readLineByte( *this, p_line, pass_useOnlyThisEndOL );
//	return _readLineByte( p_line );
	return UniFileStream_base::readLineByte_base( p_line );
}
size_t UniFileStream_file::Read(void* p_buf, size_t n_count)
{
	reOpen();
	if(this->IsClosed()) return -1;
	return pass_p_ffile->Read(p_buf, n_count);
}
const wxString& UniFileStream_file::GetName() const
{
	if(!pass_p_ffile) return wxEmptyStringWx;
	return pass_p_ffile->GetName();
}
bool UniFileStream_file::IsOpened() const
{
	if(!pass_p_ffile) return false;
	return pass_p_ffile->IsOpened();
}
wxFileOffset UniFileStream_file::Length() const
{
//	reOpen();
	if(this->IsClosed()) return _reOpenClose_size;
	return pass_p_ffile->Length();
}
bool UniFileStream_file::Seek(wxFileOffset ofs, wxSeekMode mode )
{
	reOpen();
	if(this->IsClosed()) return false;
	return pass_p_ffile->Seek(ofs, mode);
}
wxFileOffset UniFileStream_file::Tell() const
{
//	reOpen();
	if(this->IsClosed()) return _reOpenClose_pos;
	return pass_p_ffile->Tell();
}
wxFileKind UniFileStream_file::GetKind() const
{
	if(!pass_p_ffile) return wxFileKind::wxFILE_KIND_UNKNOWN;;
	return pass_p_ffile->GetKind();
}
bool UniFileStream_file::Close()
{
	if(this->IsClosed()) return false;
	if(_writePermitted){
...
bool UniFileStream_file::Error() const
{
	if(!pass_p_ffile) return true;
	return pass_p_ffile->Error();
}
void UniFileStream_file::_free()
{
	resetEol();
	resetLine();
...
bool UniFileStream_file::reOpen()
{
	if(!pass_p_ffile) return false;
...
bool UniFileStream_file::reClose()
{
...
size_t UniFileStream_file::Write(const void* pBuff, size_t nCount)
{
	 //prepare for Write
...
bool UniFileStream_file::setEndOfFile()
{
...
bool UniFileStream_string::readLineByte( wxString *p_line )
{
	if(!p_line) p_line= pass_p_line;
	if(!p_line) return false;
	return readLineByte( p_line );
}
size_t UniFileStream_string::Read(void* p_buf, size_t n_count)
{
	if(this->IsClosed()) return -1;

	if( (wxFileOffset)(_lastPos+n_count)> Length() ){
...
const wxString& UniFileStream_string::GetName() const
{
	return wxEmptyStringWx;
}
bool UniFileStream_string::IsOpened() const
{
	return( p_buf_in );
}
wxFileOffset UniFileStream_string::Length() const
{
	if(this->IsClosed()) return -1;
	return p_buf_in->size();
}
bool UniFileStream_string::Seek(wxFileOffset ofs, wxSeekMode mode)
{
	if(this->IsClosed()) return false;
	...
	if( (wxFileOffset)_lastPos> this->Length() ) {
		_lastPos= this->Length();
//		return false;
	}
	return true;
}
wxFileOffset UniFileStream_string::Tell() const
{
	if(this->IsClosed()) return -1;
	return _lastPos;
}
wxFileKind UniFileStream_string::GetKind() const
{
	return wxFileKind::wxFILE_KIND_UNKNOWN;
}
bool UniFileStream_string::Close()
{
	resetEol();
	resetLine();
	if(this->IsClosed()) return false;
...
bool UniFileStream_string::Error() const
{
	return !IsOpened();
}

size_t UniFileStream_string::Write(const void *pBuf, size_t nCount)
{
	if(IsOpened() && can_write()){
		if( (wxFileOffset)_lastPos== Length()) //we are at the end, we can just append
...
bool UniFileStream_string::setEndOfFile()
{
	if(IsOpened() && can_write()){
...
bool UniFileStream_Binary::readLineByte( wxString *p_line )
{
	if(!p_line) p_line= pass_p_line;
	if(!p_line) return false;
	return readLineByte( p_line );
}
size_t UniFileStream_Binary::Read(void* p_buf, size_t n_count)
{
	if(this->IsClosed()) return -1;
...
const wxString& UniFileStream_Binary::GetName() const
{
	return wxEmptyStringWx;
}
bool UniFileStream_Binary::IsOpened() const
{
	return( p_buf_in );
}
wxFileOffset UniFileStream_Binary::Length() const
{
	if(this->IsClosed()) return -1;
	return size;
}
bool UniFileStream_Binary::Seek(wxFileOffset ofs, wxSeekMode mode)
{
	if(this->IsClosed()) return false;
...
wxFileOffset UniFileStream_Binary::Tell() const
{
	if(this->IsClosed()) return -1;
	return _lastPos;
}
wxFileKind UniFileStream_Binary::GetKind() const
{
	return wxFileKind::wxFILE_KIND_UNKNOWN;
}
bool UniFileStream_Binary::Close()
{
	resetEol();
	resetLine();
	if(this->IsClosed()) return false;
...
bool UniFileStream_Binary::Error() const
{
	return !IsOpened();
}
...
UniFileStream_base::PAlocItemManagement::~PAlocItemManagement()
{
  if(item){
...
UniFileStream_arr::~UniFileStream_arr()
{
	FreeSources();
}
void UniFileStream_arr::FreeSources()
{
...
void UniFileStream_arr::Add(UniFileStream_base* baseClass, BYTE ownsAdd, bool DontAddSize)
{
...
void UniFileStream_arr::Add(wxFFile& file, BYTE ownsAdd, bool openReadOnly_)
{
	UniFileStream_file* temp_push= new UniFileStream_file(file, nullptr, pass_useOnlyThisEndOL);
...
bool UniFileStream_arr::readLineByte( wxString *p_line )
{
	if(!p_line) p_line= pass_p_line;
	if(!p_line) return false;
...
//void UniFileStream_arr::syncAllEOL()
//{
//	for( size_t temp_u=0; temp_u<arrSources.size(); temp_u++ ){
//		arrSources.at(temp_u)->pass_useOnlyThisEndOL= pass_useOnlyThisEndOL;
//	}
//}
size_t UniFileStream_arr::Read(void* p_buf, size_t n_count)
{
	arrSources.at(P_lastPosT)->item->reOpen();
	if(this->IsClosed()) return -1;
	if( wxFileOffset(P_lastPos+n_count)> Length() ){	//we cant read over the size of file(s)
	...
	while(true){
		...
		try{
			...
			temp_n_count= invalid_size;
		}
		if( temp_n_count!= invalid_size ){	//we really dont want to add (-1)
			n_read+= temp_n_count;
		} else {
...
const wxString& UniFileStream_arr::GetName() const
{
	if(P_lastPosT< arrSources.size()) return arrSources.at(P_lastPosT)->item->GetName();
	return wxEmptyStringWx;
}
bool UniFileStream_arr::IsOpened() const
{
...
wxFileOffset UniFileStream_arr::Length() const
{
	return P_DataLength;
}
void UniFileStream_arr::seekResetFrom(size_t indx, bool toState, bool reCloseAfter)
{
...
bool UniFileStream_arr::plus_P_lastPosT()
{
	++P_lastPosT;
...
bool UniFileStream_arr::SeekFromHere(wxFileOffset ofs)
{
	if(!ofs){	// as in Seek+0
		arrSources.at(P_lastPosT)->item->reOpen();
		return true;
...
bool UniFileStream_arr::Seek(wxFileOffset ofs, wxSeekMode mode)
{
...
wxFileOffset UniFileStream_arr::Tell() const
{
	return P_lastPos;
}
wxFileKind UniFileStream_arr::GetKind() const
{
	return arrSources.at(P_lastPosT)->item->GetKind();
	//what kind, kind of what???
}
bool UniFileStream_arr::Close()
{
	FreeSources();
return 1;
}
bool UniFileStream_arr::Error() const
{
...
void UniFileStream_part::Set_ownPos(bool state)
{
	B_OwnPos.changeBit(0,state);
}
void UniFileStream_part::use(UniFileStream_base* Part, const wxFileOffset& posFrom, const wxFileOffset& posTo )
{
	Close();
	partOf= Part;
	usePart(posFrom, posTo);
}
void UniFileStream_part::use(wxFFile& FilePart, const wxFileOffset& posFrom, const wxFileOffset& posTo )
{
	Close();
	UniFileStream_base* tempAdd= new UniFileStream_file(FilePart, pass_p_line, pass_useOnlyThisEndOL);
	B_OwnPos.changeBit(1,1);
	use(tempAdd, posFrom, posTo);
}
void UniFileStream_part::usePart(const wxFileOffset& posFrom, const wxFileOffset& posTo )
{
...
UniFileStream_part::~UniFileStream_part()
{
	Close();
}

bool UniFileStream_part::readLineByte( wxString *p_line )
{
	if(!p_line) p_line= pass_p_line;
	if(!p_line || !partOf) return false;
	return UniFileStream_base::readLineByte_base( p_line );
}
size_t UniFileStream_part::Read(void* p_buf, size_t n_count)
{
	if( wxFileOffset(P_posCurr+ n_count)> Length() ){
...
const wxString& UniFileStream_part::GetName() const
{
	if(this->IsClosed()) return wxEmptyStringWx;
	return partOf->GetName();
}
bool UniFileStream_part::IsOpened() const
{
	return partOf;
}
wxFileOffset UniFileStream_part::Length() const
{
	if(this->IsClosed()) return invalidFileOffset;
	return P_OffsEnd- P_OffsStart;
}
bool UniFileStream_part::Seek(wxFileOffset ofs, wxSeekMode mode)
{
	if(this->IsClosed()) return false;
...
wxFileOffset UniFileStream_part::Tell() const
{
	if(this->IsClosed()) return invalidFileOffset;
	return P_posCurr;
}
wxFileKind UniFileStream_part::GetKind() const
{
	if(this->IsClosed()) return wxFILE_KIND_UNKNOWN;
	return partOf->GetKind();
}
bool UniFileStream_part::Close()
{
	//cleanup if used new
...
bool UniFileStream_part::Error() const
{
...
void UniFileStream_TextFile::init(size_t maxSizeAllowed)
{
...
size_t UniFileStream_TextFile::init_readAllLines()
{
	if( _readsInput->Length() > (wxFileOffset)_maxSize ) return -1;
...
size_t UniFileStream_TextFile::Write()
{
	//write to owned file
  ...
  #if _WIN32 == 1
	_readsInput->useEOL( EOL::Windows );
  #endif // _WIN32

	return _WriteTo( toFile_w );
}
size_t UniFileStream_TextFile::WriteTo(const wxString& FileTo)const
{
...
size_t UniFileStream_TextFile::_WriteTo(wxFFile& writableFFile)const
{
	writableFFile.Seek(0);
	size_t ret_written= 0;
	wxStringVector::const_iterator atLineIter= Lines.begin();
...
//#include "unifyNumber_lenght.h"
size_t UniFileStream_fileArr::makeNextPart(bool existingOnly, bool forceOverwrite)
{
//	currFile= P_lastPosT;
	...
	wxString newFileName= fileBaseName+ fileBaseNameAddition+ unifyNumber_lenght( currFile, filePartKeepLen);
...
//	UniFileStream_arr::Add(newFFile, PAM_owns_all, forceOverwrite);
	UniFileStream_arr::Add(newFFile, PAM_owns_all, openReadOnly);
//	P_lastPosT= currFile;
return currFile;
}

size_t UniFileStream_fileArr::Write(const void *pBuf, size_t nCount)
{
...
//	fileThisPartSize
	...
	while( nCount ){
		bool notFit= 0;
		UniFileStream_file* writeTo= nullptr;
...
//			UniFileStream_base* tempClas= arrSources.at(tempu)->item;
...
wxFileOffset UniFileStream_fileArr::setToCurrFileBegin()
{
	wxFileOffset retVal= arrSources.at(P_lastPosT)->item->Tell();
	arrSources.at(P_lastPosT)->item->Seek(0);
	return retVal;
}
bool UniFileStream_fileArr::Seek(wxFileOffset ofs, wxSeekMode mode, bool doNotAddSize)
{
	if(doNotAddSize) return UniFileStream_arr::Seek( ofs, mode );
	if( mode== wxFromStart ){
		return SeekMore(ofs);
	} else if( mode== wxFromCurrent ){
...
bool UniFileStream_fileArr::SeekMore(wxFileOffset pos)
{
...
//		reOpenFile( *fWriteHand.back() );
		...
		return UniFileStream_arr::Seek(pos);
	}
return 1;
}
bool UniFileStream_fileArr::Flush()
{
	bool overall= 0;
	UniFileStream_file* pointToFile= nullptr;
	for(size_t tempU=0; tempU<arrSources.size(); ++tempU){
...
void UniFileStream_fileArr::init_PartSize(const wxFileOffset& initSize)
{
...
void UniFileStream_fileArr::init_PartSizeToCurrLenght()
{
...
void UniFileStream_fileArr::init_fileBaseNameAddition(const wxString& str)
{
...
bool UniFileStream_fileArr::Open(const wxString& pathName, bool readOnly)
{
	...
	UniFileStream_arr::Add(newFFile, PAM_owns_all);

//	P_DataLength+= newFFile->Length();
return 1;
}
bool UniFileStream_fileArr::Close_save(bool save)
{
...
bool UniFileStream_fileArr::Close_ForceDrop()
{
	UniFileStream_arr::Close();
	P_DataLength= P_lastPos= 0;
	currFile= 0;
...
UniFileStream_fileArr::UniFileStream_fileArr(const wxString& filePathName, const wxString& filePathNameAdd, const wxFileOffset& _filePartMaxSize, bool8 _AtClose, bool _openReadOnly)
{
...
size_t UniFileStream_fileArr::findLoadAllFiles(bool makeNewOnFullSize, size_t maxParts)
{
...
//printf("start P_DataLength... %u\n", P_DataLength);
	...
	while( makeNextPart(1)!= invalid_size && maxParts ) {
...
bool UniFileStream_fileArr::setEndOfFile()
{
	bool retVal= 0;
...
//			arrSources.at(tempu)= nullptr;	//null it
			wxString newFileName= fileBaseName+ fileBaseNameAddition+ unifyNumber_lenght( tempu, filePartKeepLen);
			if( wxFileExists(newFileName) ){
...
//		fWriteHand.resize( currFile+1 );
	...
	P_DataLength= 0;
	UniFileStream_base* tempClas= nullptr;
	for(size_t tempu=0; tempu<arrSources.size(); ++tempu){
...
std::string WxStringToMultibyte(const wxString& str)
{
	...
	return std::string(asUtf.data(), asUtf.length());
}
wxString WxStringFromMultibyte(const std::string& str)
{
