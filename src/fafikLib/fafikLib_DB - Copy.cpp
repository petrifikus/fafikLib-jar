...
//printf("[ table\n");
//PrintTable( &DB_interpreter_table );
//printf("] end\n");
...
fafikLib_DB::itemEntry* fafikLib_DB::tableInsertSubTable(itemEntry *table, wxString key, wxString value, BYTE type_stores)
{
	...
	return table->Add_back( key, value, type_stores);
...
//		itemEntry &temp_ie1= (*DB_interpreter_table.at(temp_inter))["opensFiles"];
		itemEntry *temp_ie1= DB_interpreter_table.at(temp_inter)->findP("opensFiles", ie_Table);
...
fafikLib_DB::itemEntry* fafikLib_DB::itemEntry::Add( BYTE type_stores_, const wxString& value_ )
{
	...
	return Add( wxEmptyString, value_, type_stores_ | ie_auto_key_keyed );
...
bool fafikLib_DB::itemEntry::_operLowerB_compareIE( iterator item, const itemEntry& compa )
{
	//return ( a < b)
		///cast to Editor readable form
	const itemEntry& inFind= **item;

	if( inFind.isLore() < compa.isLore() )	//test pos is NOT Lore, compa is Lore (1<0),(0<1)
		return true;
//	if( inFind.isTable() < compa.isTable() )	//test pos is NOT table, compa is table (1 < 0)
//		return true;
	//then search for string
		///compares key or value
	const wxString *str_a,*str_b;
		///single char
	wxString str_ac('0'),str_bc('0');
//	if( compa.key.empty() && inFind.key.empty() ){
//		str_a= &inFind.value;
//		str_b= &compa.value;
//	} else {
		str_a= &inFind.key;
...
//	}
	...
	while( strCurrPos< strLen ) {
		...
		if( compa.isCaseInsensitive() ){
			str_ac.LowerCase();
			str_bc.LowerCase();
		}

		if( str_ac< str_bc ) return true;
		if( str_ac> str_bc ) return false;
		++strCurrPos;
	}
...
//	if( compa.key.empty() && inFind.key.empty() ) //Key=="" compare by Value
//		return inFind.value.IsSameAs(compa.value, !compa.isCaseInsensitive() );
