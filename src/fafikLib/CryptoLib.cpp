...
#pragma GCC diagnostic ignored "-Wparentheses" //suppress the stupid warning that come from public lib

#include "CryptoLib.h"

using namespace fafik77;

const int32_t CryptoLib::_keyLocation{31820158};

bool CryptoLib::encryptData_7(LPCVOID dataIn, LPVOID dataOut, const size_t &dataSize, LPCVOID KeyIn, const size_t &KeySize)
{
...
bool CryptoLib::decryptData_7(LPCVOID dataIn, LPVOID dataOut, const size_t &dataSize, LPCVOID KeyIn, const size_t &KeySize)
{
		//dont accept errors
...
const BYTE CryptoLib::_v7_encodeKey_clas::constPredef[16]={
	0x1a, 0x23, 0x05, 0x73,
	0xf2, 0xfa, 0x3c, 0x5b,
	0x4a, 0x76, 0xf4, 0x68,
	0x92, 0x81, 0x41, 0xe5
};
BYTE CryptoLib::_v7_encodeKey_clas::ShiftR(const BYTE& inB, const BYTE& by) const {
	return inB>> by;
}
BYTE CryptoLib::_v7_encodeKey_clas::RotR(const BYTE& inB, const BYTE& by) const {
	return (inB>> by) | (inB<< (8- by));
}
BYTE CryptoLib::_v7_encodeKey_clas::CH(const BYTE& x, const BYTE& y, const BYTE& z) const {
	return (x & y) ^ (~x & z);
}
BYTE CryptoLib::_v7_encodeKey_clas::MAJ(const BYTE& x, const BYTE& y, const BYTE& z) const {
	return (x & y) ^ (x & z) ^ (y & z);
}
BYTE CryptoLib::_v7_encodeKey_clas::F1(const BYTE& x) const {
	return (RotR(x, 2) ^ RotR(x, 5) ^ RotR(x, 6));
}
BYTE CryptoLib::_v7_encodeKey_clas::F2(const BYTE& x) const {
	return (RotR(x, 6) ^ RotR(x, 3) ^ RotR(x, 1));
}
BYTE CryptoLib::_v7_encodeKey_clas::F3(const BYTE& x) const {
	return (RotR(x, 7) ^ RotR(x, 2) ^ RotR(x, 3));
//	return (RotR(x, 1) ^ RotR(x, 4) ^ RotR(x, 3));
}
BYTE CryptoLib::_v7_encodeKey_clas::F4(const BYTE& x) const {
	return (RotR(x, 1) ^ RotR(x, 3) ^ RotR(x, 2));
}

	///1 Block = 16 Bytes
void CryptoLib::_v7_encodeKey_clas::encode_inBlocks(const BYTE* dataIn, const size_t &BlocksAmount)
{
	BYTE dataWrite[48];
...
void CryptoLib::_v7_encodeKey_clas::update(const BYTE* dataIn, const size_t &dataSize)
{
...
void CryptoLib::_v7_encodeKey_clas::finish()
{
	if(_b_finished) return;
	_b_finished= true;
...
void CryptoLib::_init(){
}
BYTE CryptoLib::shorten_32to8(const size_t &dataIn)const
{
...
void CryptoLib::_v7_encodeKey(const BYTE* data_in, encodedKey& data_out, const size_t& dataSize, size_t& HashedOut){
...
BYTE CryptoLib::_v7_encodeKey_Sum(const BYTE* dataIn, const size_t &dataSize)
{
...
void CryptoLib::_v7_shuffleKey(BYTE* dataIO, const size_t &dataSize, const size_t &dataShuffle)
{
	if(!dataSize || dataSize== size_t(-1) || dataSize== size_t(1)) return;
		//cast to type
		//make
	BYTE FirstChar= dataIO[1];
	std::vector<BYTE> dataIO_Copy(dataSize);
	memcpy(&dataIO_Copy.at(0), dataIO, dataSize);
	...
	for(size_t startPos=0; startPos!=dataSize; ++startPos){
		...
		//make sure to destroy sensitive data
	FirstChar= 0x00;
	memset(&dataIO_Copy.at(0), 0x00, dataSize );
	reinterpretPos= 0;
}
	///do some magic to make the data even more unpredictable
size_t CryptoLib::_v7_SuperSecretKey_gen(const size_t &dataSize, const BYTE* KeyEncodedIn, const size_t &KeySize, const BYTE& KeyEncodedSume)
{
...
void CryptoLib::_v7_shuffleData(BYTE* dataIO, const size_t &dataSize, const size_t& SuperSecret_key, bool reverseOp)
{
...
void CryptoLib::_DataOp_MozaicData(BYTE* dataIO, const size_t &dataPosStart, const size_t &dataPosEnd) //0
{
...
void CryptoLib::_DataOp_ReverseData(BYTE* dataIO, const size_t &dataPosStart, const size_t &dataPosEnd) //1
{
	size_t dataDiffSize= dataPosEnd- dataPosStart;
	dataDiffSize&= ~size_t(1); //loose the 1, num&(~1)
	if(!dataDiffSize || dataDiffSize== 1) return;
...
void CryptoLib::_DataOp_AddData(BYTE* dataIO, const size_t &dataPosStart, const size_t &dataPosEnd, const BYTE& conAddData) //2
{
...
void CryptoLib::_DataOp_RotateData(BYTE* dataIO, const size_t &dataPosStart, const size_t &dataPosEnd) //3
{
	size_t dataDiffSize= dataPosEnd- dataPosStart;
	if(!dataDiffSize || dataDiffSize== 1) return;
...
void CryptoLib::_v7_encrData_p1(const BYTE *dataIn_B, BYTE *dataOut_B, const size_t &dataSize, encodedKey& KeyEncodedIn, const BYTE &SSKeyHashShort)
{
	if(!dataSize || dataSize== size_t(-1)) return;
...
//	dataOut_B= NULL;
	...
	///@param dataIn.size == dataOut.size, otherwise error
void CryptoLib::_v7_decrData_p1(const BYTE *dataIn_B, BYTE *dataOut_B, const size_t &dataSize, encodedKey& KeyEncodedIn, const BYTE &SSKeyHashShort)
{
	if(!dataSize || dataSize== size_t(-1)) return;
		//give it a type(unsigned), otherwise any operation is invalid
		//key
...
#pragma GCC diagnostic pop //very handy tool indeed
