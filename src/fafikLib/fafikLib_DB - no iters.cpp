...
{
	this->DB_interpreter_table= DB_interpreter_table;
 ...
 */
 ...
 if(DB_interpreter_table.sizeTable()==0){
	//tableInsert(&DB_interpreter_table, "fafikLib_CON" );	///Compacted Object Notation
	tableInsert(&DB_interpreter_table, "main interpreter table" );	///Pointer Recursive Object Notation
	itemEntry *temp_ins[5];
		//fafikLib_PRON
	temp_ins[0] = tableInsertSubTable(&DB_interpreter_table, "fafikLib_PRON" );
...
//	   temp_ins[3] = tableInsertSubTable(temp_ins[2], "e", "]" );
	 ...
	 temp_ins[1] = tableInsertSubTable(temp_ins[0], "opensFiles", "", ie_auto_key );
	  ...
	  temp_ins[2] = tableInsertSubTable(temp_ins[1], "", "json", ie_auto_key_keyed );
		...
		//ini files
	temp_ins[0] = tableInsertSubTable(&DB_interpreter_table, "INI" );
	 temp_ins[1] = tableInsertSubTable(temp_ins[0], "assign" );
	  temp_ins[2] = tableInsertSubTable(temp_ins[1], "e", "=" );
	 temp_ins[1] = tableInsertSubTable(temp_ins[0], "key_value" );
...
//printf("  koko 3.3 = %i\n",tempSubN.length());
//				wxString(tempSubN).ToLong(&temp_L) ;
...
//	}
	...
	curr_interpreter.entry_table = getItem_byKey_rec(
	 interpreterTable_p,translate_StrToArr("table"),0,0);
...
BYTE fafikLib_DB::Open(wxString file_path, modes_IO mode, const wxString overwriteInterpreter, settings *useSettings, bool clearsPrevious)
{
	this->Close();
...
{
	if(DB_fileIO.IsOpened()) DB_fileIO.Close();
...
{
	wxLogNull noLog;
	const wxString temp_fileName= DB_fileIO.GetName();
...
int fafikLib_DB::WriteToFile_interpreter( wxString filePathName )
{
	...
	int ret_ok= WritePToFile( &DB_interpreter_table );
...
{
//	wxLogNull noLog;
	DB_file_write.Open( DB_fileIO.GetName()+".new", "w+b" );
  if( (!DB_file_write.IsOpened()) || DB_file_write.Error()){
		DB_file_write.Close();
		wxRemoveFile( DB_fileIO.GetName()+".new" );
		return 1;
  } else {
	...
	if( !ret_ok ){
		...
		DB_fileIO.Close();
		wxLogNull noLog;
//		wxLogMessage("from . to .old");
		wxRenameFile(temp_fileName, temp_fileName+".old");
//		wxLogMessage("from .new to .");
		wxRenameFile(temp_fileName+".new", temp_fileName);

			//ok 1,1
			//fail 1,0
			//f fail 0,0
//		wxLogMessage("renames 1,2 = %u,%u", rename1, rename2);
...
{
	if( !wxIsWritable( DB_fileIO.GetName() ) ) return 0;
	DB_file_write.Open( DB_fileIO.GetName(), "ab" );
	int ret_ok= WritePToFile();
return ret_ok;
}
...
//printf(" isLink;\n");
	...
	DB_file_write.Write( DB_fileIOLine.getEOL() );
...
BYTE fafikLib_DB::UseInterpreter(const wxString& interName)
{
	...
	itemEntry* temp_ie1= DB_interpreter_table.find(interName);
	if( temp_ie1 ){
...
//wxLogMessage("picked inter = %i", temp_inter);
...
{
	if(file_path.empty()) return -1;
//printf("fafikLib_DB temp_DB_interFile\n");
//getchar();
	fafikLib_DB temp_DB_interFile(this->DB_interpreter_table);
//printf("temp_DB_interFile.DB_interpreter_table=\n");
//getchar();
//	temp_DB_interFile.DB_interpreter_table.Free();
//	temp_DB_interFile.DB_interpreter_table= this->DB_interpreter_table;
///new idea 2018-08-09
//	temp_DB_interFile.DB_interpreter_table.linksTo= &this->DB_interpreter_table;
//	temp_DB_interFile.DB_interpreter_table.type_stores|= ie_link;
	BYTE tempB= temp_DB_interFile.Open(file_path, mode_read, overwriteInterpreter);
//printf("tempB = %i\n",tempB);
//getchar();
	if(tempB) return -1;
	temp_DB_interFile.interpretFile();
	temp_DB_interFile.DB_interpreter_table.Detach();
//printf("\n\n============= now combine DB_Table with interpreter ==================\n");
//printf("size = %i\n",temp_DB_interFile.DB_table.sizeTable());

	int retAdded= 0;
	for(size_t temp_u=0; temp_u<temp_DB_interFile.DB_table.sizeTable(); temp_u++){
		itemEntry *retItem= itemEntry_getExistOrNew( DB_interpreter_table, *temp_DB_interFile.DB_table.at(temp_u) );
		if(retItem){
//printf("retItem exists...........\n");
		} else {
//printf("retItem does not exist...........\n");
			retAdded++;
			this->DB_interpreter_table.push_back_cpyTotal( *temp_DB_interFile.DB_table.at(temp_u) );
//printf("...added %i\n",temp_u);
//wxLogMessage("adding %s= %s", temp_DB_interFile.DB_table.at(temp_u)->key, temp_DB_interFile.DB_table.at(temp_u)->value);
		}
	}

//printf("\n now DB_interpreter_table.sizeTable = %i\n", DB_interpreter_table.sizeTable());
//printf("\n\n============= ended with ==================\n");

return retAdded;


//	this->DB_interpreter_table
//	temp_DB_interFile.DB_table;
}
bool fafikLib_DB::itemEntry_isSame(itemEntry& item_a, itemEntry& item_b)
{
...
wxLongLong fafikLib_DB::interpretFile()
{
...
wxArrayString fafikLib_DB::translate_StrToArr(wxString findKey_levelRecursive_str, const wxUniChar separator)
{
...
fafikLib_DB::itemEntry* fafikLib_DB::getItem_byKey_rec(itemEntry &Table, wxArrayString findKey_levelRecursive, size_t currPos, BYTE is_interpreter)
{
...
//printf("using %%n???????????????????????????????????????????????????\n");
				...
				if(temp_pos1> pos_last+iMDA){
					if( temp_pos1==size_t(-1) && !temp_tab_uu->args_L.empty()){
						CanExitLoopOr= true;
						mkS.interpreted_value= wxLongLong(temp_tab_uu->args_L.Item(0)).ToString();
						mkS.pos_from= pos_last;
						mkS.pos_to= pos_last;
						pos_last++;
					} else
...
//printf("temps_strSB = |%s|\n", temps_strSB.ToStdString().c_str());
				...
				if( temps_strSB.ToLongLong(&temp_LL) ){
					...
					wxLongLong temp_wLL(temp_LL);
					mkS.interpreted_value= temp_wLL.ToString();
...
//	itemEntry *temp_ie_p= new itemEntry;
	...
	if(optionBytes && optionBytes->length()!= 0)
		DB_fileIO.Seek( optionBytes->pos_end );
...
//printf("koko e.1 end\n");
	...
	temp_ie.type_stores|= ie_Table;
	if(optionBytes && temp_ie.ByteData_itemEntry_selfEntry){
		ByteData_itemEntry_all.pop_back();
		optionBytes= nullptr;
...
fafikLib_virtualByteFile* fafikLib_DB::interpretNextLine_TBytes(wxString &in_line, itemEntry* Table, wxVector<makesSearch> &lineInterpreterResults, size_t &pos_last )
{
...
//			continue;
	...
	if(byte_type && (temp_LL>=0 && temp_LL<=8) ){ //byte_type of long assign(number of bytes containing data length)
		if(temp_LL== 0){
//			temp_vBf.length= 0;
			temp_vBf.pos_start= DB_fileIO.Tell();
			temp_vBf.pos_end= temp_vBf.pos_start;//+ temp_vBf.length;
		} else {
			BYTE temp_chars[8]= {0};
			size_t read1= DB_fileIO.Read( &temp_chars[0], temp_LL);
			if(read1== size_t(temp_LL)){
				long long *temp_LL2_p= static_cast<long long*>(static_cast<void*>(temp_chars) );
//				temp_vBf.length= *temp_LL2_p;
				temp_vBf.pos_start= DB_fileIO.Tell();
				temp_vBf.pos_end= temp_vBf.pos_start+ *temp_LL2_p;
				temp_LL2_p= nullptr;
//				if(temp_vBf.length!= 0)
//					DB_fileIO.Seek( temp_vBf.length, wxFromCurrent );
			} else {
wxLogError("Could not load %i Bytes from DB file, pos= %s.", temp_LL, wxLongLong(DB_fileIO.Tell()).ToString() );
			}
		}
	} else {
//		temp_vBf.length= temp_LL;
		temp_vBf.pos_start= DB_fileIO.Tell();
		temp_vBf.pos_end= temp_vBf.pos_start+ temp_LL;
//		if(temp_vBf.length!= 0)
//			DB_fileIO.Seek( temp_vBf.length, wxFromCurrent );
	}
...
BYTE fafikLib_DB::interpretNextLine()	//itemEntry* Table
{
//[[ new code to activate 2018-08-08
	if(!_NextLine_iE) return -1; //one type of error
//	here->replace-All( Table -->> _NextLine_iE )
//]] end new code
...
fafikLib_DB::itemEntry* fafikLib_DB::itemEntry::find(const wxString &item_byKeyName, bool caseInsensitive, size_t* outFoundPos)
{
	for(size_t temp_u=0; temp_u<this->sub_itemTable.size(); temp_u++)
	{
		if( item_byKeyName.IsSameAs( this->sub_itemTable.at(temp_u)->key, (!caseInsensitive) ) ){
...
//if(depth< size_t(10))
//{
//	for(size_t temp_u=0; temp_u<depth; temp_u++)
//		printf(" ");
//}
//printf("~itemEntry  # %u , %u ,, %u\n", depth, type_stores, bool(parent) );
...
void fafikLib_DB::itemEntry::setSubsParent()
{
	setSubsParent(this);
}
...
{
	itemEntry *temp_IE= new itemEntry( key_, value_, type_stores_, this->depth+1 );
//	temp_IE->key =key_;
//	temp_IE->value =value_;
//	temp_IE->type_stores =type_stores_;
//	temp_IE->depth= this->depth+1;
	if( (!value_.empty()) && !(type_stores_& ie_lore) ){
		temp_IE->type_stores |= ie_value;
	}
//./..

//printf("conpon add() type = %i = |%s|\n", temp_IE->type_stores, temp_IE->key.ToStdString().c_str() );
//getchar();


	return this->push_back_ptr( temp_IE );
}

fafikLib_DB::itemEntry* fafikLib_DB::itemEntry::Add( const wxString& key_ )
{
	itemEntry *temp_IE= new itemEntry( key_, wxEmptyString, 0, this->depth+1 );
//	temp_IE->key =key_;
//	temp_IE->depth= this->depth+1;
...
fafikLib_DB::itemEntry* fafikLib_DB::itemEntry::Add( BYTE type_stores_, const wxString& value_ )
{
	...
	itemEntry *temp_IE= new itemEntry( wxEmptyString, value_, ie_auto_key_keyed, this->depth+1 );
//	temp_IE->value =value_;
//	temp_IE->type_stores|= ;
//	temp_IE->depth= this->depth+1;
...
fafikLib_DB::itemEntry::iterator fafikLib_DB::itemEntry::getOrAdd( const wxString& key_, BYTE type_, bool caseInsensitive, bool *out_found= nullptr)
{
	...
	//add
	if( out_found ) *out_found= false;
//	sub_itemTable.insert( atPos, templItem);	//add it, lock it
...
{
	if( by_pos< this->sizeTable() ){
		if(this->sub_itemTable.at(by_pos)){	//do not enter nullptr
			this->sub_itemTable.at(by_pos)->parent= nullptr;	//do not call back
			this->sub_itemTable.at(by_pos)->~itemEntry();
		}
		if(silent){
			this->sub_itemTable.at(by_pos)= nullptr;
		} else {
			this->sub_itemTable.erase(by_pos);
		}
		return true;
	}
return false;
...
size_t fafikLib_DB::itemEntry::pop_back(size_t amount)
{
	size_t retVal= 0;
	while(amount){
		if(sizeTable() ){
			removeItem(sizeTable()-1, false);
			//sub_itemTable.pop_back();		//inline{ erase(end() - 1); }
			++retVal;
		} else break;
		--amount;
	}
...
{
//	size_t temp_u= pos_from;
printf("itemEntry::removeItems -->> temp_u = %i\n", amount);

	for(size_t temp_u=amount; temp_u>0; --temp_u ){

printf("itemEntry::removeItems -->> temp_u = %i\n", temp_u);
printf("itemEntry::removeItems -->> item = %i\n", pos_from-temp_u);
//		this->sub_itemTable.erase( pos_from, pos_from+(amount-1) );
	}
return 0;
}
bool fafikLib_DB::itemEntry::removeItems(const itemEntry* by_id, size_t amount, bool silent)
{
	for(size_t temp_u=0; temp_u<amount; temp_u++){
...
fafikLib_DB::itemEntry* fafikLib_DB::itemEntry_iterate::find( wxString &findByStr, bool caseInsensitive )
{
	itemEntry aw;
//	itemEntry wwwww;
//	fafikLib_DB::get
...
//	itemEntry_arrP;
	...
	for(size_t temp_u=0; temp_u<findByManyStr.size(); temp_u++){
		temp_recFind= temp_recFind->find(findByManyStr.Item(temp_u), caseInsensitive);
...
bool fafikLib_DB::itemEntry::_operLowerB_compareIE( wxVector<fafikLib_DB::itemEntry*>::iterator item, const itemEntry& compa )
{
	//return ( a < b)
		///cast to Editor readable form
	const itemEntry& inFind= **item;

	if( inFind.isTable() < compa.isTable() )	//test pos is NOT table, compa is table
		return true;
	//then search for string
	return inFind.key.IsSameAs(compa.key, !compa.isCaseInsensitive() );


//	inFind.isTable() == compa.isTable()	//ok same go to string part
////	inFind.isTable() > compa.isTable()	//test pos is table but our Compa is not a table ERROR
//	inFind.isTable() < compa.isTable()	//test pos is NOT table, Compa is table
////	a				 < b
////	ie_Table < ie_value( ~ie_Table )
...
//		auto thatIt= std::lower_bound( sub_itemTable.begin(), sub_itemTable.end(), TemplateItem, fafikLib_DB::itemEntry::_operLowerB_compareIE );
//if( thatIt!= DF_list_p.end() && (*thatIt)->getPathName()== temp_findEnt.getPathName() ){
//		return thatIt- DF_list_p.begin_f();
//	} else {
////	for( auto iterItem= sub_itemTable.begin(); iterItem!= sub_itemTable.end(); ++iterItem ){
////
////	}
//	for(size_t temp_u=0; temp_u<this->sub_itemTable.size(); temp_u++)
//	{
...
bool fafikLib_DB::itemEntry::binaryGetPosOf_exists(iterator prevRetValue, const itemEntry& TemplateItem)const
{
	if(!prevRetValue) return false;
	if(sub_itemTable.end()== prevRetValue) return false;
